...
public class Financiamento_2Fragment extends Fragment {
    ...
    private final TextWatcher textWatcher = new TextWatcher() {
        ...
        public void onTextChanged(CharSequence s, int start, int before, int count) {
            ...
            String _valor_financiado = "";

            if (Locale.getDefault().getLanguage().equals("pt")){
                _valor_financiado = valor_financiado.getText().toString().replaceFirst("\\s", "").replace(".", "").replace(",", ".").trim();
                _valor_parcela = valor_da_parcela.getText().toString().replaceFirst("\\s", "").replace(".", "").replace(",", ".").trim();
            } else {
                _valor_financiado = valor_financiado.getText().toString().replaceFirst("\\s", "").replace(",", "").trim();
                _valor_parcela = valor_da_parcela.getText().toString().replaceFirst("\\s", "").replace(",", "").trim();
            }

            calculaFinanciamento_2(_valor_parcela,_valor_financiado,periodo.getText().toString(), taxa_de_juros, valorTotalPagar, valorGastoComJuros, taxaAnualJuros);
        }
