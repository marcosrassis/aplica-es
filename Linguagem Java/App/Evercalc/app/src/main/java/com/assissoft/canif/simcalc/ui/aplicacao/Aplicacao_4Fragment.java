...
public class Aplicacao_4Fragment extends Fragment {
    ...
    private final TextWatcher textWatcher = new TextWatcher() {
        ...
        public void onTextChanged(CharSequence s, int start, int before, int count) {
            ...
            String _capital = "";
            String _valor_futuro = "";

            if (Locale.getDefault().getLanguage().equals("pt")){
                _capital = capital.getText().toString().replaceFirst("\\s", "").replace(".", "").replace(",", ".").trim();
                _valor_futuro = valor_futuro.getText().toString().replaceFirst("\\s", "").replace(".", "").replace(",", ".").trim();
            } else {
                _capital = capital.getText().toString().replaceFirst("\\s", "").replace(",", "").trim();
                _valor_futuro = valor_futuro.getText().toString().replaceFirst("\\s", "").replace(",", "").trim();
            }

            calculaAplicacao_4(_valor_futuro,periodo.getText().toString(),_capital, taxa_de_juros);
        }
